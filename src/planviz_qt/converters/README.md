...
print(f".venv/bin/python run.py --map {map_path} --plan {output_path} --format planviz")
PY
```

Use the printed command to open the normalized JSON in the viewer. The built-in `planviz` converter reads that file even though the example converter is not registered in the default application. With the package installed, the same Python API works without `PYTHONPATH=src`.
...
```sh
.venv/bin/python run.py --list-formats
PYTHONPATH=src .venv/bin/python -m planviz_qt.converters --help
PYTHONPATH=src .venv/bin/python -m planviz_qt.converters normalize \
...
PYTHONPATH=src .venv/bin/python -B -m unittest discover \
  -s tests -p 'test_converter_example.py' -v
PYTHONPATH=src .venv/bin/python -B -m unittest discover \
  -s tests -p 'test_input_formats.py' -v
PYTHONPATH=src .venv/bin/python -B -m unittest discover \
  -s tests -p 'test_exchange.py' -v
```
