...
This branch contains the PySide6 implementation of PlanViz at the repository
root. The original Tkinter viewer is preserved in the `v3.2.0` Git tag.
Screenshots below are captures of the actual Qt widgets using
...
root, the directory containing `run.py`, `pyproject.toml`, `src/`, and `example/`.
...
The repository root is an installable Python project:
...
`python run.py`. Example files are in this repository, not bundled in the
...
```sh
python run.py --map example/random-32-32-20.map --plan example/mapf_plan_example.json
```
...
```sh
python run.py --map example/warehouse_small.map --plan example/warehouse_small_2024.json --hm example/warehouse_small_2024.json
```
...
`PYTHONPATH=src python -m planviz_qt.converters ...` on macOS/Linux. On
PowerShell, set `$env:PYTHONPATH = "src"` before `python -m planviz_qt.converters ...`.
...
```sh
python run.py --map example/warehouse_small.map --plan example/warehouse_small_2026.json --start 100 --end 2000 --n 50 --fps 60 --speed 0.5 --grid true --aid true --tid false --static
```

### Inspect a large input without a GUI

```sh
python run.py --inspect --map example/LoRR2026/maps/scene_mp_2p_01.map --plan example/LoRR2026/outputs/iron-example_10000_output.json
```
...
The Qt application does not accept those four Tkinter-specific flags. Check out
the `v3.2.0` tag in a separate checkout if their exact behavior is required.
Video/GIF recording is
...
PlanViz/
  ...
  example/                     Shared maps, execution logs and converter inputs
  src/planviz_qt/
...
| [Migration notes](docs/MIGRATION.md) | Root layout, updated commands and the original Tkinter release |
...
```sh
QT_QPA_PLATFORM=offscreen MPLCONFIGDIR=/tmp/planviz-qt-mpl PYTHONPATH=src python -B -m unittest discover -s tests -v
```

On PowerShell, set these environment variables with `$env:NAME = "value"` before
running `python -B -m unittest discover -s tests -v`; choose a writable local
...
uses the actual dialogs/renderers, and writes PNGs to `docs/images/`. It sets
