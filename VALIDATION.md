...
This records the initial migration and subsequent validation work. The initial
implementation lived in `qt/`, alongside the original application. On
2026-09-09 the `qt-migration` branch promoted that implementation to the
repository root; the original Tkinter release remains in the `v3.2.0` tag.
Reproduction commands below use the current root layout. No Tkinter imports or
dependencies on the original application's modules occur in the Qt package.
...
The `qt-migration` branch first committed the complete Qt implementation, then
moved it to the repository root. All production Python source files are
byte-identical to that checkpoint; changes to tests and tooling update their
repository-relative paths. The Tkinter entry points, manuals and screenshots
were removed from this branch. Shared examples and the original license remain.

- **Tests:** all 165 existing tests passed in 20.078 seconds using Python 3.12.13
  and PySide6 6.11.1 with the offscreen platform.
- **Source launcher:** `python run.py --inspect` loaded the 200-agent 2026
  warehouse fixture and reported finite initial, middle and final states.
- **Documentation:** 63 relative file links and heading anchors resolved. All
  14 screenshots and the fixture hashes in their provenance manifest remained
  valid after moving them to `docs/images/`.
- **Packaging:** wheel and source distributions built from the root
  `pyproject.toml`. A wheel installed into a temporary target provided working
  `planviz-qt --inspect` and `planviz-convert --help` commands from outside the
  repository, with its palette and converter schema included. An extracted
  source distribution contained the app, documentation, tools, tests and shared
  examples, and its own `run.py --inspect` command succeeded.
- **Preservation:** the `main` reference, existing personal files and previously
  staged `Test.py` entry were unchanged by the migration.

Build tools and install verification targets were placed under `/tmp`; existing
virtual environments were not modified. These checks validate the new layout
and packaging, not native Windows/Linux desktop behavior.
